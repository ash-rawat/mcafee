...
checkForIncompatibleProduct()
{
    ...
    then
        ...
        if [ "${returnValue}" -eq 1 ]
        then
            logMessage "Existing $productDesc ${INSTALLED_PROD_VERSION_INT} is incompatible with McAfee Endpoint Security for Linux Threat Prevention 10.7.6-43. Aborting installation."
...
PACKAGED_MFE_TP_VERSION_INT="10.7.6.43"
...
installerFile="${currentDir}/McAfeeTP-10.7.6-43-standalone.linux.tar.gz"
...
then
    ...
    MFE_TP_PACKAGE_FILE="${tmpDir}/McAfeeTP-10.7.6-43.deb"
else
    logMessage "Detected rpm based distribution"
    UNINSTALL_CMD="rpm -e "
    # Format is McAfeeTP-8.0.0-1600.x86_64.rpm
    MFE_TP_PACKAGE_FILE="${tmpDir}/McAfeeTP-10.7.6-43.x86_64.rpm"
fi
...
# Only for a fresh installation, invoke ESP installer script to install runtime, ESP, ESP for FileAccess and ESP Arbitrary Access Control
    ...
    if [ -f ${tmpDir}/McAfeeESP-KernelModule-10.7.6-121-Full.linux.tar.gz ]
    then
        bash ${tmpDir}/validate-mfeesp.sh ${installType} ${log} ${pkgMgrDir} ${tmpDir}/McAfeeESP-Basic-10.7.6-121-Full.linux.tar.gz ${tmpDir}/McAfeeESP-KernelModule-10.7.6-121-Full.linux.tar.gz
    else
        bash ${tmpDir}/validate-mfeesp.sh ${installType} ${log} ${pkgMgrDir} ${tmpDir}/McAfeeESP-Basic-10.7.6-121-Full.linux.tar.gz ${currentDir}/McAfeeESP-KernelModule-10.7.6-121-Full.linux.tar.gz
    fi
